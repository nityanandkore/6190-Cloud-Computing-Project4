...
def fetch_keyphrase(row):
    ...
    LOG.debug(f"Found Sentiment: {payload}")
    keyphrase = payload['KeyPhrases']
    return keyphrase

def apply_keyphrase(df, column="wikipedia_snippit"):
    """Uses Pandas Apply to Create Sentiment Analysis"""

    df['Keyphrase'] = df[column].apply(fetch_keyphrase)
    return df
...
def lambda_handler(event, context):
    ...
    # Fetch Key Phrasaes
    #df = apply_keyphrase(df)
    #LOG.info(f"KeyPhrases from FANG companies: {df.to_dict()}")

    # Write result to S3
    write_s3(df=df, bucket="nkfangsentiment", name=names)